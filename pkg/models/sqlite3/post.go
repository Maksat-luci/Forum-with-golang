...
// InsertPost inserts post to db
func (c *Database) InsertPost(p *models.Post) (int64, error) {
...
func (c *Database) AddPostCategory(categories []int, postID int64) error {
...
func (c *Database) 	GetAllPosts() (*[]models.Post, error) {
	...
	title, posts.time_creation FROM posts INNER JOIN people ON posts.user_id = people.user_id ORDER BY posts.post_id DESC;`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
// GetPostByPostID gets post content
...
// GetPostsByUserID gets all the posts created by "user ID"
...
func (c *Database) GetPostsByReaction(emotion, userID int) (*[]models.Post, error) {
	rows, err := c.SqlDb.Query(`SELECT posts.post_id, people.username, posts.title, posts.content, posts.time_creation FROM posts
		...
		WHERE post_reaction.reaction = ? AND post_reaction.user_id = ?;`, emotion, userID)
	if err != nil {
		return nil, err
	}
